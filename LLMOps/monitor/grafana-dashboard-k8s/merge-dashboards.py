...
def main():
    ...
    vllm_perf = load_json("vllm-performance-statistics.json")
    vllm_query = load_json("vllm-query-statistics.json")

    namespace_dashboard_vars(sglang, "sgl", {"model_name", "namespace", "pod"})
    namespace_dashboard_vars(vllm_perf, "vllm", {"model_name", "namespace", "pod"})
    namespace_dashboard_vars(vllm_query, "vllm", {"model_name", "namespace", "pod"})
    ...
    with open("../llm-all-k8s.json", "w") as f:
        json.dump(merged, f, indent=2, ensure_ascii=False)

    # Summary
    total_panels = len([p for p in all_panels if p.get("type") != "row"])
    print(f"Merged dashboard written to llm-all-k8s.json")
